...
import com.casabaca.common.ejb.model.ArccrdPK;
...
import com.casabaca.cxc.ejb.dto.CxcHistoricoPagosView;
...
@ManagedBean
@ViewScoped
public class HistoricoPagosController extends CommonController implements Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = -814100187550621978L;
	private static final Logger LOG = Logger.getLogger(HistoricoPagosController.class);
	private static final String REPORTE_HIST_PAGOS = "/reportes/historicoPagos.jasper";
	private static final String NOMBRE_REPORTE_HIST_PAGOS = "historicoPagos.pdf";
	...
	private List<Arccrd> arccrds;
	private Arccrd documentoSeleccionado;
	private HistoricoDocEntradaDto historicoDocEntradaDto;

	public HistoricoPagosController() {
	...
	public void init() {
		...
		Object[] datosClienteFac = nativeService.consultarDatosClienteFactura(historicoDocDto);
		cedulaClienteFac = "0";
		nombreClienteFac = "**";
		if (datosClienteFac != null) {
			cedulaClienteFac = String.valueOf(datosClienteFac[0]);
			nombreClienteFac = String.valueOf(datosClienteFac[1]);
		}
		cargarHistPagos();
	}
	...
	public void onRowSelect(SelectEvent event) {
		documentoSeleccionado = (Arccrd) event.getObject();
	}
	...
	public void generarReporte() {
		...
		try {
			...
			HttpServletResponse response = (HttpServletResponse) FacesContext.getCurrentInstance().getExternalContext()
					.getResponse();
			String ctxPath = getServletContext().getRealPath("/") + REPORTE_HIST_PAGOS;
			...
			response.setHeader("Content-Disposition", "attachment;filename=\"" + NOMBRE_REPORTE_HIST_PAGOS + "\"");
	...
	private void cargarHistPagos() {
		if (("3".equals(historicoDocDto.getSerieFisico()) || "6".equals(historicoDocDto.getSerieFisico())
				|| "4".equals(historicoDocDto.getSerieFisico()))) {
			List<CxcHistoricoPagosView> datosVista = nativeService.getHistoricoPagosPorNoFisico(noCia,
					String.valueOf(historicoDocDto.getNoCliente()), historicoDocDto.getAgencia(),
					historicoDocDto.getNoFisico(), historicoDocDto.getSerieFisico(),
					String.valueOf(historicoDocDto.getCantProrroga()));
			arccrds = new ArrayList<>();
			int i = 1;
			for (CxcHistoricoPagosView cxcHistoricoPagosView : datosVista) {
				Arccrd arccrd = new Arccrd();
				arccrd.setArccrdPk(new ArccrdPK(this.noCia, "a", String.valueOf(i)));
				arccrd.setNroDocumento(cxcHistoricoPagosView.getNoDocuRefe() == null ? 0: Integer.parseInt(cxcHistoricoPagosView.getNoDocuRefe()));
				arccrd.setFechaVence(cxcHistoricoPagosView.getFechaVence());
				arccrd.setFecha(cxcHistoricoPagosView.getFechaCancelacion());
				arccrd.setCentro(cxcHistoricoPagosView.getAgencia());
				arccrd.setNotaIngreso(cxcHistoricoPagosView.getNotaIngreso());
				arccrd.setMonto(cxcHistoricoPagosView.getValorTotal());
				arccrd.setInteres(cxcHistoricoPagosView.getValorInteres());
				arccrd.setDescuento(cxcHistoricoPagosView.getValorDescuento());
				arccrd.setTotalPago(cxcHistoricoPagosView.getValorTotal());
				arccrd.setDias(cxcHistoricoPagosView.getDias());
				arccrds.add(arccrd);
				i++;
			}
		} else {
			Arccmd arccmd = new Arccmd();
			arccmd.setId(new ArccmdPK(this.noCia, null));
			arccmd.setCentro(historicoDocDto.getAgencia());
			arccmd.setSerieFisico(historicoDocDto.getSerieFisico());
			arccmd.setNoFisico(historicoDocDto.getNoFisico());
			arccmd.setCantProrrogas(BigDecimal.valueOf(historicoDocDto.getCantProrroga()));
			arccmd.setFecha(historicoDocDto.getFecha());
			arccmd.setNoCliente(historicoDocDto.getNoCliente().toString());
			arccrds = historicoDocumentosService.consultarHistorialPagos(arccmd);
		}
		if (arccrds != null && !arccrds.isEmpty()) {
			Comparator<Arccrd> datosOrdena = Comparator.comparing(Arccrd::getNroDocumento)
					.thenComparing(Arccrd::getFecha);
			arccrds = arccrds.stream().sorted(datosOrdena).collect(Collectors.toList());
		}
		consultarDatosCliente();
		documentoSeleccionado = arccrds != null && arccrds.size() > 0 ? arccrds.get(0) : null;
	}
	...
	public List<Arccrd> getArccrds() {
		return arccrds;
	}

	public void setArccrds(List<Arccrd> arccrds) {
		this.arccrds = arccrds;
	}
	...
	public Arccrd getDocumentoSeleccionado() {
		return documentoSeleccionado;
	}

	public void setDocumentoSeleccionado(Arccrd documentoSeleccionado) {
